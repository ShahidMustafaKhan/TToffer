...
class _OfferChatScreenState extends State<OfferChatScreen> {
  ...
  void initState() {
    ...
    predefinedMessages = getMessages(widget.product?.category?.name  == 'Jobs' ? widget.product?.productType ?? '' : widget.product?.category?.name ?? '');
  ...
  getUserDetails() async {
    if(widget.participantModel == null){
      ...
      UserModel? participant = await userViewModel.getSellerProfile(int.parse(widget.receiverId.toString()));
...
//! offer variables
  ...
  Future<void> getConversation({bool loading=false}) async {
   ...
   conversation = await chatApi.getConversationList(widget.conversationId, loading: loading);
    ...
    if(conversation.isNotEmpty){
      ...
      widget.receiverId ??= conversation.last.receiverId == userId ? conversation.last.senderId : conversation.last.receiverId  ;
    }
   ...
   getUserDetails();
    ...
    if(conversation.isNotEmpty){
      if(conversation.last.receiverId == userId && widget.conversationId != null){
      ...
      Consumer<ChatViewModel>(
          builder: (context, chatViewModel, child) {
            List<Conversation> list = conversation.reversed.toList();
              return chatViewModel.conversationLoading == true ? Center(child: SizedBox(height: 50, width: 50 ,child: CircularProgressIndicator(color: AppTheme.yellowColor,)))
                  : Stack(
                    children: [
                      Column(
                        ...
                        children: [
                          ...
                          Expanded(
                            child: (conversation.isEmpty) && predefinedMessages.isNotEmpty && chatViewModel.conversationLoading == false && (widget.isOffer == true && userId.toString() == widget.sellerId.toString()) == false
                                ? Padding(
                                  padding: EdgeInsets.symmetric(horizontal: 18.w, vertical: 12.h),
                                  child: chatViewModel.sendingTapMessage == true ? Center(child: SizedBox(height: 50, width: 50 ,child: CircularProgressIndicator(color: AppTheme.yellowColor,))) :Column(
                                                ...
                                                Flexible(
                                                  child: Container(
                                                    ...
                                                    child: AppText.appText(
                                                      ...
                                                      fontSize: 13.0,
                                                      textColor: const Color(0xff6ba99c),
  ...
  Widget offerDialog(){
    return widget.isOffer == true && userId.toString() == widget.sellerId.toString()
        ? Align(
      child: Padding(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Stack(
                    ...
                    children: [
                      Container(
                        constraints: BoxConstraints(maxWidth: systemMessage(message ?? '') ? (user ? MediaQuery.of(context).size.width * 0.82 : MediaQuery.of(context).size.width * 0.89) : 250, minWidth: 80),
                        ...
                        if(user ? currentUserImage!=null : participantModel?.img!=null)
                        Positioned(
                          right: user ? -8.w : null,
                          left: user ? null : -8.w,
                          top:  user ? -6.h : -6.h,
                          child: CircleAvatar(
                            radius: 10.r,
                            backgroundImage: NetworkImage(user ? currentUserImage! : participantModel!.img!),
                          ),
                        )
                    ],
                  ),
          ...
          Stack(
            ...
            children: [
              ClipRRect(
                  ...
                  child: Image.file(imageFile!, height: 120.w, width: 120.w, fit: BoxFit.cover)),
              Positioned(
                right: -10,
                top: -10,
                child: InkWell(
                ...
                if (chatViewModel.sendingMessage)
                  ...
                  InkWell(
                    onTap: () async {
                      ...
                      if(imageFile != null){
                        sendImage(imageFile!.path, userMessage.isNotEmpty ? userMessage : null);
                      }
                      else{
  ...
  Future<bool> sendFileMsg(
    ...
    chatViewModel.sendMessageWithImage(senderId: senderId, receiverId: receiverId, buyerId: buyerId, sellerId: sellerId, productId: productId, filePaths: image.path, message: message)
        .then((value){
