...
class UserViewModel with ChangeNotifier {
  ...
  Future<UserModel?> getSellerProfile(int? sellerId) async {
    try {
      ...
      final response = await profileRepository.getSellerProfileApi(sellerId, data);
      return response;
    } catch(e){
  ...
  Future<void> updateUserProfileAndImage(int? userId, String imagePath) async {
    try {
      ...
      final response = await profileRepository.updateCompleteProfile(data , imagePath);
      setUserModel(ApiResponse.completed(response.userModel));
  ...
  Future<void> toggleWishList(int? userId, int? productId, BuildContext context, {int? itemIndex}) async {
    if(itemIndex != null){
  ...
  Future<void> addSavedItem(int? userId, int? productId, BuildContext context, {int? index, int? saveItemIndex}) async {
    ...
    try {
      ...
      Provider.of<CartViewModel>(context, listen: false).removeCartItem(productId, userId);
      savedItemLoading(false, context, index: index, saveItemIndex: saveItemIndex);
  ...
  Future<void> deleteSavedItem(int? id, BuildContext context, {int? index, int? saveItemIndex}) async {
    savedItemLoading(true, context, index: index, saveItemIndex: saveItemIndex);
    try {
  ...
  void savedItemLoading(bool value, BuildContext context, {int? index, int? saveItemIndex}){
    if(index != null){
      Provider.of<CartViewModel>(context, listen: false).updateToggleSaveLoadingValue(value, index);
    }
  ...
  Future<void> updateVerification(int? userId, {bool verifyPhone = false} ) async {
    try {
  ...
  Future<void> addProductReview(dynamic data, int? productId) async {
    ...
    try {
      ...
      final response = await profileRepository.addProductReview(data,productId);
      setReviewLoading(false);
      return response;
  ...
  Future<void> overViewApi(int? userId, BuildContext context) async {
    try {
      ...
      Provider.of<CartViewModel>(context, listen: false).setCartItemsCount(response.cartItemCount ?? 0);
      Provider.of<ChatViewModel>(context, listen: false).toggleUnReadMessageIndicator((response.chatCount ?? 0) != 0) ;
      Provider.of<NotificationProvider>(context, listen: false).setIndicatorCount(response.unreadNotifications ?? 0);
  ...
  logout(BuildContext context, userId){
    ...
    Provider.of<CartViewModel>(context, listen: false).setCartList(ApiResponse.notStarted());
    Provider.of<ChatListViewModel>(context, listen: false).setBuyingChat(ApiResponse.loading());
    Provider.of<ChatListViewModel>(context, listen: false).setSellingChat(ApiResponse.loading());
    Provider.of<SellingPurchaseProvider>(context, listen: false).sellingProductsModel = null;
